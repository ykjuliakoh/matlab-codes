function vector=GII_LR2(X,Y,Z,theta_hat,epsilon,beta)
lambda_2=0.02;N=length(X);
p=length(theta_hat);K=length(beta);
...
for m=1:M
    smoothed_u2(:,m)=exp(simulated_u2(:,m)/lambda_2)./(ones(N,1)+exp(simulated_u2(:,m)/lambda_2)+exp(simulated_u3(:,m)/lambda_2));
    smoothed_u3(:,m)=exp(simulated_u3(:,m)/lambda_2)./(ones(N,1)+exp(simulated_u2(:,m)/lambda_2)+exp(simulated_u3(:,m)/lambda_2));
end
...
for k=1:K
    ...
    denominator=(ones(N,M)+exp(simulated_u2./lambda_2)+exp(simulated_u3./lambda_2)).^2;
    coeff_A=(1/lambda_2)*(ones(N,M)+exp(simulated_u3./lambda_2)).*exp(simulated_u2./lambda_2);
    coeff_B=(-1/lambda_2)*exp(simulated_u2./lambda_2).*exp(simulated_u3./lambda_2);
    coeff_C=(1/lambda_2)*(ones(N,M)+exp(simulated_u2./lambda_2)).*exp(simulated_u3./lambda_2);
    if k<=3
